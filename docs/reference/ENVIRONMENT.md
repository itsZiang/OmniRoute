...
| `ALLOW_API_KEY_REVEAL`                  | `true`                  | `src/shared/constants/featureFlagDefinitions.ts` | Allows revealing full API key values in the Dashboard UI. Configurable from Dashboard Feature Flags. Defaults to `true` so operators of small, self-hosted instances can copy existing keys any time; set to `false` for shared/multi-user instances where stored keys should stay masked.                                                                                                                                                                                                                            |
| `ALLOW_KEYPOOL_AUTOREPLACE`             | `true`                  | `src/shared/constants/featureFlagDefinitions.ts` | When `true`, a quota/credits-exhausted upstream API key connection is automatically replaced with a fresh key from that provider's key pool (Dashboard > Providers > [id] > Key Pool). Configurable from Dashboard Feature Flags. Defaults to `true` for small self-hosted instances; set to `false` to require manual "Pull from pool". |
